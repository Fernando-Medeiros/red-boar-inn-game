...
export default async function (form: UpdatePasswordSchema, token?: string, compMessage?: string) {
    const { message, statusCode } = await PasswordService.reset(form, String(token))
